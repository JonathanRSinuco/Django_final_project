...
def show_exam_result(request, course_id, submission_id):
    ...
    for choice in choices:
        ...
        if question.id not in procesed_questions:
            procesed_questions.append(question.id)
            get_score = question.grade if question.is_get_score(choices) else 0
            max_score += question.grade
            total_score += get_score

    contex = {}
    contex["grade"] = total_score * 100 // max_score
    contex["course"] = course
    contex["selected_choices"] = choices
    return render(request, "onlinecourse/exam_result_bootstrap.html", contex)
